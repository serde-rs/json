...
fn test_arbitrary_precision() {
    ...
    for s in cases {
        ...
        let nv = to_value(&n).unwrap();
        assert_eq!(v, nv);

        let vv = to_value(&v).unwrap();
        assert_eq!(v, vv);

        let ns = to_string(&n).unwrap();
        assert_eq!(s, &ns);

        let vs = to_string(&v).unwrap();
        assert_eq!(s, &vs);

        let nd = Number::deserialize(n.clone()).unwrap();
        assert_eq!(n, nd);

        let vd = Value::deserialize(v.clone()).unwrap();
        assert_eq!(v, vd);

        let ndv = Number::deserialize(v.clone()).unwrap();
        assert_eq!(n, ndv);

        let vdn = Value::deserialize(n.clone()).unwrap();
        assert_eq!(v, vdn);
    }
}
